...
int main()
{
	...
	do {
		...
		if (userHand.deadPoints > compHand.deadPoints) {
			...
			cout << "The computer now has " << compScore << " points." << endl;
		}
			...
			cout << "This round was a tie. Congrats, you are now as smart as a computer." << endl;
		}
...
bool AIPick(Hand &h, stack <Card> &draw, Card &discard) {
	...
	discard = (h.all.at(h.all.size() - posOfVal));
	h.all.erase(h.all.end() - posOfVal + 1);
